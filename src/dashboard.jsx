  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
        <div style={{ fontSize: 11, color: 'var(--text-2)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: 0.4 }}>
          {label}
        </div>
...
const DimensionCard = ({ dim, data, indicators, publisher, country, onDrilldown }) => {
  ...
  const series = (() => {
    if (dim.id === 'economic') return [
      { key: 'gdp', label: 'GDP Growth', value: indicators[0]?.values[publisher] ?? indicators[0]?.values.adb, unit: '%' },
      { key: 'cpi', label: 'Inflation', value: indicators[1]?.values[publisher] ?? indicators[1]?.values.adb, unit: '%' },
      { key: 'debt', label: 'Debt/GDP', value: indicators[2]?.values[publisher] ?? indicators[2]?.values.adb, unit: '%' },
    ];
    if (dim.id === 'social') return [
      { key: 'pov', label: 'Poverty Rate', value: indicators[0]?.values[publisher] ?? indicators[0]?.values.adb ?? indicators[0]?.values.wb, unit: '%' },
    ];
    return [];
  })();
    ...
    <div className="panel-flat" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {series.length > 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {series.map((s, i) => (
            <TrendChart key={s.key} data={ts[s.key] || []} years={window.GenieData.YEARS}
    ...
    <div className="panel-flat" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <h1 className="serif" style={{ margin: 0, fontSize: 44, fontWeight: 400, letterSpacing: -0.5, lineHeight: 1 }}>
            {country.name}
          </h1>
        ...
        <div style={{ display: 'flex', gap: 18 }}>
          ...
          <Stat label="Risk Posture" value="High" tone="risk"/>
        </div>
      </div>
    </div>
  );
...
const Stat = ({ label, value, tone }) => (
  <div>
    <div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 4 }}>{label}</div>
    <div className="mono" style={{ fontSize: 22, fontWeight: 600, color: tone === 'risk' ? '#ff8b8b' : 'white' }}>{value}</div>
  </div>
);
...
const Dashboard = ({ country, publisher, onDrilldown, role, onSelectCountry }) => {
              ...
              return (
                <button key={id} onClick={() => onSelectCountry && onSelectCountry(id)}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12 }}>
                    <div>
                      <div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 2 }}>GDP growth</div>
  ...
  const activeDimensions = DIMENSIONS.filter(d => !d.soon && country.dimensions[d.id] && (INDICATORS[d.id] || []).length > 0);
  const comingDimensions = DIMENSIONS.filter(d => d.soon || !INDICATORS[d.id] || INDICATORS[d.id].length === 0);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: 18, height: '100%', overflow: 'auto' }}>
      ...
      <div className="panel-flat" style={{ padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <Icon name="sparkles" size={14} style={{ color: 'var(--text-2)' }}/>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-1)' }}>Future Dimensions</div>
          <span className="mono" style={{ fontSize: 10, color: 'var(--text-3)', marginLeft: 'auto' }}>roadmap</span>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {comingDimensions.map(d => (
            <div key={d.id} style={{
              display: 'flex', alignItems: 'center', gap: 8,
              padding: '8px 12px', background: 'var(--hover-tint)',
              border: '1px solid var(--line)', borderRadius: 8, opacity: 0.6,
            }}>
              <Icon name={d.icon} size={14} style={{ color: d.color }}/>
              <span style={{ fontSize: 12 }}>{d.label}</span>
              <span className="mono" style={{ fontSize: 10, color: 'var(--text-3)' }}>Q3 '26</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="panel-flat" style={{ padding: 14, fontSize: 11, color: 'var(--text-2)', lineHeight: 1.65 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>Sources used on this page</span>
        </div>
